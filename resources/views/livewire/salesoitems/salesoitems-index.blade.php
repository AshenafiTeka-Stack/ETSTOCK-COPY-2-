<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Sales Order Items ') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Manage all your Sales Order Items') }}</flux:subheading>
...
<div>
   ...
   @endsession
    ...
    <a href="{{ route('salesoitems.create') }}"
    class="cursor-pointer px-3 py-2 text-xs font-medium text-white bg-green-700 rounded-lg">Create Sales Order Items</a>
...
<div class="mt-4 overflow-x-auto">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tr>
            ...
            <th scope="col" class="px-6 py-3">Sales Order ID</th>
        ...
        <tbody>
            ...
            @foreach ($salesoitems as $salesoitem )
                
        <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
            <td class="px-6 py-2 font-medium text-gray-900 dark:text-white">{{ $salesoitem->id }}</td>
            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $salesoitem->so_id }}</td>
            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $salesoitem->product_id }}</td>
            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $salesoitem->quantity }}</td>
            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $salesoitem->unit_price }}</td>
           
            <td class="px-6 py-2 flex gap-2">
                
                <a href="{{ route('salesoitems.show', $salesoitem->id) }}"
                ...
                <a href="{{ route('salesoitems.edit', $salesoitem->id) }}"
                ...
                <button wire:click="delete({{ $salesoitem->id }})"
                    wire:confirm="Are you sure, you want to remove this sales order item?"
