<div>
    <div class="relative mb-6 w-full">
        ...
        <flux:subheading size="lg" class="mb-6">{{ __('Manage all your Inventories') }}</flux:subheading>
...
<div>
   ...
   @endsession
    ...
    class="cursor-pointer px-3 py-2 text-xs font-medium text-white bg-green-700 rounded-lg">Create Inventory</a>
...
<div class="mt-4 overflow-x-auto">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
            <td class="px-6 py-2 font-medium text-gray-900 dark:text-white">{{ $inventory->id }}</td>
            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $inventory->product_id }}</td>
            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $inventory->warehouse_id }}</td>
            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $inventory->quantity }}</td>
            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $inventory->last_updated }}</td>
            ...
            <td class="px-6 py-2 flex gap-2">
                ...
                <button wire:click="delete({{ $inventory->id }})"
                    wire:confirm="Are you sure, you want to remove this inventory?"
